...
function Cart() {
  ...
  const handlePayment = async () => {
    try {
      const pharmacistData = await axios.get(
        "http://localhost:3005/getPharmacist",
        {}
      );
      ...
      if (paymentMethod === "payWithVisa") {
        ...
        await handleOrder();
        window.location =
          paymentMethod === "payWithWallet" ? "/Cancel" : "/Success";
      }
    } catch (error) {
      console.error("Error updating data:", error);
      ...
      if (
        ...
        error.response.data === "Not enough to remove"
      ) {
        ...
        <div className="flex mb-14 mt-32">
          <div className="ml-10">
            {CartData.map((p, index) => (
              <div
                key={index}
                className="mt-6 w-[60rem] h-[14.25rem] rounded-md shadow-md  bg-gray-100 justify-center space-y-4"
              >
                <div className="justify-center pl-4 pt-4">
                  {p.Picture ? (
                    <img alt={p.Name} className="w-20 h-20 mt-16" />
                  ) : (
                    <div className="mt-4 rounded-md shadow-md w-40 h-40 bg-gray-300">
                      <br></br>no image
                    </div>
                  )}
                  <br />
                  <div className="-mt-44 ml-44 mb-4">
                    ...
                    <div className="-mt-6">
                      <label className="text-gray-500 ml-64"> Each: </label>
                      <label className="font-bold ">EGP </label>
                      <label>
                        <a>{p.price}</a>
                      </label>
                      <label className="text-gray-500 ml-64"> Total: </label>
                      <label className="font-bold ">EGP </label>
                      <label>
                        <a>{p.totalprice}</a>
                      </label>
                      ...
                      <div className="mt-8 ml-34">
                        ...
                        <button
                          className="justify-end text-red-600 outline w-9 h-9 rounded-md mb-2 mt-0.5 ml-2"
                          onClick={() =>
                            handledecrement(p.medicineName, p.price)
                          }
                        >
                          -
                        </button>
                        <button
                          className={`justify-end text-sky-600 outline w-9 h-9 rounded-md mb-2 mt-0.5 ml-3 ${
                            p.count <= 0 ? "cursor-not-allowed" : ""
                          }`}
                          onClick={() =>
                            handleincrement(p.medicineName, p.price)
                          }
                          disabled={p.count <= 0}
                        >
                          +
                        ...
                        <button
                          className="justify-end ml-80 pl-52"
                          onClick={() => handleremove(p.medicineName)}
                        >
                          <FontAwesomeIcon
                            icon={faTrashCan}
                            size="lg"
          ...
          <div className="ml-4 mt-6 w-[30rem] h-[30rem] bg-gray-50 rounded-md shadow-md justify-center space-y-4">
            <div className="flex items-center mt-4 ml-4">
              <select
                ...
                className="bg-50 border border-300 text-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-48 p-2.5"
              >
            ...
            <form className="ml-4 pt-4">
              ...
              <label className="text-gray-500">
                <input
                  ...
                  name="paymentMethod"
                  value="payWithWallet"
                  checked={paymentMethod === "payWithWallet"}
                  onChange={handlePaymentMethodChange}
                />{" "}
                Pay with Wallet
              </label>
              ...
              <label className="mt-2 text-gray-500">
                <input
                  ...
                  name="paymentMethod"
                  value="payWithVisa"
                  checked={paymentMethod === "payWithVisa"}
                  onChange={handlePaymentMethodChange}
                />{" "}
                Pay with Visa
              </label>
              {paymentMethod === "payWithWallet" && (
                <div className="pt-4">
                  <label className="text-gray-500 ml-4">
                    {" "}
                    Points in wallet: {randomPointsInWallet}{" "}
                  </label>
                  <br />
                  <label className="text-gray-500 ml-4"> Total: {total} </label>
                  <br />
                  <label className="text-gray-500 ml-4">
                    {" "}
            ...
            <div className="pt-4">
              <label className="text-gray-500 ml-4"> Shipping cost </label>
              <label className="text-gray-500 ml-72"> TBD </label>
              <br></br>
              <label className="text-gray-500 ml-4"> Tax </label>
              <label className="text-gray-500 pl-12 ml-80"> TBD </label>
              <br></br>
              <label className="text-gray-500 font-bold ml-4">
                {" "}
                Total price{" "}
              </label>
              <label className="text-gray-500 font-bold pl-72 ml-4">
                {" "}
                {total}{" "}
              </label>
            </div>
            <div className="flex justify-end mt-10">
              ...
              <a>
                <button
                  className="justify-end bg-sky-600 text-white w-60 h-10 rounded-md mb-2 mt-0.5 mr-14"
      ...
      <div className="Bootstrap PatientHome">
        ...
        <div className="mt-40">
          <div className="h-[16rem] justify-center text-center space-y-4 mt-28">
            ...
            <h3>
              Return Home <a href="/PatientHome">here</a>
            </h3>
          </div>
        </div>
      </div>
